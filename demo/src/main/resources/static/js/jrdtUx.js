/**
 * ux.js 복사하고, datepicker 관련 부분만 주석처리
 */
...
$(function(){
	...
	$(".board-content tbody").find("tr>td").hover(
		function(){
			...
			if ($this.parent().parent().parent().find("table").length) {return;}
	...
	$sideMenu.click(function(e){
		...
		if ($sub.is(":hidden")) {
			...
			$this.removeClass("active").parent().removeClass("current");
	...
	*/
	...
	$(".btn_add_typ1").on("click",function() {
		var arrRow = [
			'<tr>',
				'<td class="contents">',
					'<p class="intbox"><span>RCFM : </span><input type="text"></p>',
				'</td>',
	...
	$(".sel_currency","#sidebar").change(function(){
		var $this = $(this), $cvc = $(".txt_cvc,.ip_cvc","#sidebar");
	...
	/* ffp */
	$(".side_ffp .tab li","#sidebar").click(function(){
		var $this = $(this),
			$li = $(".side_ffp .tab li","#sidebar").removeClass("current").filter($this).addClass("current").end();
		var idx = $li.index($this); 
		$(".side_ffp .tabview","#sidebar").hide().eq(idx).show();
	}).first().click();
	...
	$(".btn_add_typ3").click(function(){
		...
		$(html).appendTo($(".tbl_add_typ3").find("tbody"))
				.find(".btn_del_row").click(function(e){
											$(this).parent().parent().remove();})
				.end().find(".sData").datepicker();
